...
def Vec2Rot(xi):
    # MatrixExp3(VecToso3(xi))
    assert type(xi) == np.ndarray, f'w={xi} should be a numpy array'
    return pin.exp3(xi)
...
def Vec2SE3(lamb):
    ...
    :param lamb: se3 'vector' in list or ndarray, [v, w]
    :return: SE3 in ndarray
    """
    assert type(lamb) == np.ndarray, f'xi={lamb} should be a numpy array'
    # lamb = np.asarray(lamb).reshape(-1)
    return pin.exp6(lamb).np

def SE32Vec(T):
    return pin.log6(T).np.reshape(-1, 1)
